...
import ViewCounterExample from "./ViewCounterExample.vue";
...
<ViewCounter server:defer id={post.id}>
	<div
		slot="fallback"
		class="inline-block h-[1lh] w-[8ch] animate-pulse rounded-lg bg-neutral-800">
	</div>
</ViewCounter>
```
...
<div id="view-counter-increase" class="inline-block" data-post-id={id}>
	<div
		id="view-counter-increase__skeleton"
		class="h-[1lh] w-[8ch] animate-pulse rounded-lg bg-neutral-800">
	</div>
	<p id="view-counter-increase__content" class="empty:hidden"></p>
</div>
...
<script>
	...
	const initViewCounter = () => {
		const viewCounterIncreaseElement = document.getElementById(
			"view-counter-increase",
		);
		const viewCounterIncreaseSkeletonElement = document.getElementById(
			"view-counter-increase__skeleton",
		);
		const viewCounterIncreaseContentElement = document.getElementById(
			"view-counter-increase__content",
		);
