---
title: How to Build a Page View Counter with Astro DB and Server-side Islands
slug: how-to-build-a-page-view-counter-with-astro-db-and-server-side-islands
description: "Discover how to implement a view counter on your Astro pages without relying on public API routes to update the count, using Astro DB, Astro Actions and server-side islands for a secure and efficient solution."
createdAt: 2025-04-01
tags: [ "astro", "astro db", "astro actions", "server islands" ]
---
import {ViewCounterExample} from "./ViewCounterExample.js";
...
import { PageViews, db, eq } from "astro:db";
...
- `db.select()`: Retrieves data from the database.

- `.from(PageViews)`: Specifies the PageViews table.

- `.where(eq(PageViews.id, id))`: Filters the data where the id column matches the given id.
...
Now, we need to create a new Astro component at `src/components/ViewCounter.astro` that will display and update the view count.

```astro
---
export const prerender = false; // Disable prerendering for this component, as it's dynamic

// Prevent caching for this server island to ensure that when the user navigates back, they always see the most up-to-date content.
...
const getCount = async (): Promise<undefined | number> => {
	...
	const { data } = await Astro.callAction(actions.getPageViews, id);

	return data?.[0]?.count;
};

// For the case that we want to read an count before it got created, we need to return an default view count of 0
