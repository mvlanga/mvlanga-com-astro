...
<Image
    src={ogImage}
    class="bg-neutral-900 rounded-xl"
...
```typescript title="src/utils/open-graph.ts"
...
```typescript title="src/utils/open-graph.ts" collapse={1-17} ins={19-36}
import fs from "node:fs/promises";
import path from "node:path";
...
```typescript title="src/utils/open-graph.ts" collapse={1-2,8-40} ins={42-61}
import fs from "node:fs/promises";
import path from "node:path";
...
```typescript title="src/pages/blog/[id]/og.jpeg.ts"
...
<Alert additionalClasses="not-first:mt-12 not-last:mb-12">
    **Small reminder about styling**
    ...
    When styling the html content, please keep in mind that `Satori` doesn't support all css properties, check the docs
    for the list of supported properties [here](https://github.com/vercel/satori?tab=readme-ov-file#css).

    `Satori` also supports `tailwindcss`, but it is experimental and because it does not support all css properties and
    some are written differently (e.g. `object-fit` vs `objectFit`) I do not recommend using tailwindcss utility classes
    here.
</Alert>
...
```typescript title="src/pages/blog/[id]/og.jpeg.ts" collapse={3-55} ins={1-1, 57-64}
